...
**Static-validator and lane ordering:** `test:gate:static` declares the 13 canonical, directly runnable read-only validators. `scripts/run-static-gate-checks.mjs` starts them concurrently and waits for **every** result, so zero, one, or multiple policy failures remain fail-closed and observable before tests start. It then invokes `scripts/run-gate-lanes.mjs`, which starts `engine-core`, `test:pg-gate`, and `test:unit-gate` concurrently with Node's platform-neutral process API, waits for **all three**, and returns nonzero if any fail. On Windows it prefers the direct Corepack `pnpm.js` entrypoint (with a fixed native-command fallback when unavailable); it never invokes a POSIX shell. CI-shape runs only after that successful wait.
