...
`project.agents.roles` is a normalized JSONB role-tag array. Migration `0046_fn_8764_multi_role_workflow_agents.sql` backfills it from legacy singular roles. `workflow_work_items` also persist the routed principal fence fields used for recovery and audit.
