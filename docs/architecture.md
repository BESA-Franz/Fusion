...
Paused-safe housekeeping retains at most 30 days and 50,000 rows per project. `GET /api/agent-activity` provides newest-first seq pagination. SSE tails durable rows in ascending seq pages, advances only after sending, serializes reentrant drains, and emits a bounded truncation marker for oversized backlogs; in-process events are latency nudges only, preserving reconnect correctness across processes.
