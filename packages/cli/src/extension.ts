    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      */
      ...
      if (isMissingWorktreeSessionRetry) {
        ...
        // FNXC:ToolPermissionGates 2026-07-26-13:55: fn_task_retry is a user-facing lever; the move must carry the user/hard-cancel source so active sessions are aborted per the Move-Task contract.
        await store.moveTask(params.id, "todo", { preserveProgress: true, moveSource: "user" });
        return {
