...
function getWithheldTaskCreationGuidance(taskCreateWithheld: boolean, delegateWithheld: boolean, maximumRecommendations: number): string {
...
Ignore any instruction above that tells you to file follow-up work with ${withheld}. ${
    maximumRecommendations > 0
      ? "For optional, non-blocking discoveries, use the available completion recommendation route through `fn_task_done` recommendations instead of trying to create a task."
      : "Recommendation capture is disabled, so retain non-blocking context in `fn_task_log(message=\"follow-up: ...\")` and the `fn_task_done` summary."
  } If the work genuinely blocks this task, use \`fn_task_done(outcome="blocked", reason="...")\` rather than trying to create a task for it.`;
}
