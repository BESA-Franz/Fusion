...
describe("inspectBranchConflict ghost references", () => {
  ...
  it("returns tip-already-merged when branch tip is reachable from main despite stale startPoint", async () => {
    const repoDir = await setupRepo();
    const staleStartPoint = await git(["rev-parse", "HEAD"], repoDir);
    for (let i = 0; i < 5; i += 1) {
      await appendFile(path.join(repoDir, "note.txt"), `m${i}\n`, "utf-8");
      await git(["add", "note.txt"], repoDir);
      await git(["commit", "-m", `chore: main-${i}`], repoDir);
    }
    await git(["branch", "fusion/fn-9999"], repoDir);
    ...
    if (result.kind === "tip-already-merged") {
      ...
      expect(result.tipSha).toBe(await git(["rev-parse", "fusion/fn-9999"], repoDir));
    }
  }, 20_000);
  ...
  it("keeps genuine live-foreign conflicts unchanged", async () => {
    const repoDir = await setupRepo();
    await git(["checkout", "-b", "topic/other"], repoDir);
    await appendFile(path.join(repoDir, "note.txt"), "foreign\n", "utf-8");
    await git(["add", "note.txt"], repoDir);
    await git(["commit", "-m", "chore: foreign work"], repoDir);
    await git(["checkout", "main"], repoDir);
    const livePath = path.join(repoDir, "wt-foreign");
    await git(["worktree", "add", livePath, "topic/other"], repoDir);
