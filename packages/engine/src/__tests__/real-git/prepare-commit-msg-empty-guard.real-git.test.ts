...
function git(dir: string, args: string[]): { stdout: string; stderr: string; status: number | null } {
  try {
    const stdout = execFileSync("git", args, { cwd: dir, stdio: ["ignore", "pipe", "pipe"] }).toString();
...
describe("prepare-commit-msg empty-commit guard (real git, FN-5345/FN-5377)", () => {
  it("refuses --allow-empty in fusion task worktrees, but allows amend and real commits", async () => {
    ...
    try {
      ...
      git(rootDir, ["config", "user.email", "t@t"]);
      git(rootDir, ["config", "user.name", "t"]);
      writeFileSync(join(rootDir, "README.md"), "init\n");
      git(rootDir, ["add", "README.md"]);
      git(rootDir, ["commit", "-m", "init"]);

      git(rootDir, ["worktree", "add", "-b", "fusion/fn-5345", "wt", "HEAD"]);
      ...
      git(worktreeDir, ["add", "real.txt"]);
      const real = git(worktreeDir, ["commit", "-m", "feat(FN-5345): real"]);
      expect(real.status).toBe(0);

      // --allow-empty with new message is REFUSED.
      const empty = git(worktreeDir, ["commit", "--allow-empty", "-m", "feat(FN-5345): empty handoff"]);
      ...
      const sneaky = git(
        worktreeDir,
        ["commit", "--allow-empty", "-m", "feat(FN-5345): fix --amend handling"],
      );
      ...
      const sneakyAm = git(
        worktreeDir,
        ["commit", "--allow-empty", "-am", "feat(FN-5345): fix --amend handling via -am"],
      );
      expect(sneakyAm.status).not.toBe(0);
      expect(sneakyAm.stderr).toContain("refusing empty commit");
      const sneakyVm = git(
        worktreeDir,
        ["commit", "--allow-empty", "-vm", "feat(FN-5345): fix --amend handling via -vm"],
      );
      ...
      const legitAm = git(worktreeDir, ["commit", "-am", "feat(FN-5345): legit -am commit"]);
      expect(legitAm.status).toBe(0);

      // --amend --no-edit (no staged changes, amend HEAD) is ALLOWED.
      const amendNoEdit = git(worktreeDir, ["commit", "--amend", "--no-edit"]);
      expect(amendNoEdit.status).toBe(0);

      // --amend -m "..." (no staged changes, reword) is ALLOWED.
      const amendReword = git(worktreeDir, ["commit", "--amend", "-m", "feat(FN-5345): reworded"]);
      expect(amendReword.status, amendReword.stderr).toBe(0);

      // Outside fusion worktrees (no fusion-task-id metadata), --allow-empty works normally.
      const outside = git(rootDir, ["commit", "--allow-empty", "-m", "chore: legit empty in root"]);
