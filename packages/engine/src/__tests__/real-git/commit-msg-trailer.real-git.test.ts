...
function git(dir: string, args: string[], input?: string): string {
  return execFileSync("git", args, { cwd: dir, input, stdio: "pipe" }).toString().trim();
}
...
describe("commit-msg trailer hook (real git)", () => {
  it("appends and preserves Fusion-Task-Id trailer in fusion worktrees", async () => {
    ...
    try {
      ...
      git(rootDir, ["worktree", "add", "-b", "fusion/kb-7", "wt-kb", "HEAD"]);
      ...
      git(worktreeDir, ["add", "step1.txt"]);
      git(worktreeDir, ["commit", "-m", "feat(KB-7): first"]);
      const firstBody = git(worktreeDir, ["log", "-1", "--format=%B"]);
      ...
      const firstTrailers = git(worktreeDir, ["interpret-trailers", "--parse"], firstBody);
      ...
      git(worktreeDir, ["commit", "--amend", "--no-edit"]);
      const amendNoEditBody = git(worktreeDir, ["log", "-1", "--format=%B"]);
      ...
      git(worktreeDir, ["commit", "--amend", "-m", "feat(KB-7): rewritten"]);
      const rewrittenBody = git(worktreeDir, ["log", "-1", "--format=%B"]);
      ...
      git(worktreeDir, ["add", "step-manual.txt"]);
      git(worktreeDir, ["commit", "-m", "feat(KB-7): manual coauthor", "-m", "Co-authored-by: Fusion <noreply@runfusion.ai>"]);
      const manualBody = git(worktreeDir, ["log", "-1", "--format=%B"]);
      expect((manualBody.match(/Co-authored-by:\s*Fusion <noreply@runfusion\.ai>/g) ?? []).length).toBe(1);

      const taskFile = git(worktreeDir, ["rev-parse", "--git-path", "fusion-task-id"]);
      ...
      git(worktreeDir, ["add", "step2.txt"]);
      git(worktreeDir, ["commit", "-m", "feat(KB-7): lowercase metadata"]);
      const lowercaseBody = git(worktreeDir, ["log", "-1", "--format=%B"]);
      ...
      git(rootDir, ["add", "outside.txt"]);
      git(rootDir, ["commit", "-m", "chore: root commit"]);
      const rootBody = git(rootDir, ["log", "-1", "--format=%B"]);
  ...
  it("uses custom co-author settings and honors commitAuthorEnabled false", async () => {
    ...
    try {
      ...
      git(rootDir, ["worktree", "add", "-b", "fusion/fn-7065-custom", "wt-custom", "HEAD"]);
      await installTaskWorktreeIdentityGuard({
      ...
      git(customWorktreeDir, ["add", "custom.txt"]);
      git(customWorktreeDir, ["commit", "-m", "feat(FN-7065-CUSTOM): custom author"]);
      const customBody = git(customWorktreeDir, ["log", "-1", "--format=%B"]);
      expect((customBody.match(/Co-authored-by:\s*Fusion Bot <bot@example\.com>/g) ?? []).length).toBe(1);

      git(rootDir, ["worktree", "add", "-b", "fusion/fn-7065-disabled", "wt-disabled", "HEAD"]);
      await installTaskWorktreeIdentityGuard({
      ...
      git(disabledWorktreeDir, ["add", "disabled.txt"]);
      git(disabledWorktreeDir, ["commit", "-m", "feat(FN-7065-DISABLED): disabled author"]);
      const disabledBody = git(disabledWorktreeDir, ["log", "-1", "--format=%B"]);
