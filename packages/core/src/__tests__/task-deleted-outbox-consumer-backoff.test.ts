...
describe("TaskDeletedOutboxConsumer idle backoff", () => {
  ...
  it("wakes at the durable retry deadline instead of polling during the retry window", async () => {
    ...
    const outboxReads: number[] = [];
    mocks.readTaskLifecycleConsumerCursor.mockImplementation(async () => {
      cursorReads.push(Date.now());
      return {
        ...makeCursor(),
        retryBackoffUntil: retryActive ? new Date(retryDeadline).toISOString() : null,
      };
    });
    mocks.listTaskLifecycleEvents.mockImplementation(async () => {
      outboxReads.push(Date.now());
      retryActive = false;
      return [];
    });
    const consumer = new TaskDeletedOutboxConsumer(store);
    ...
    expect(cursorReads.length).toBeGreaterThanOrEqual(2);
    expect(outboxReads).toHaveLength(1);
