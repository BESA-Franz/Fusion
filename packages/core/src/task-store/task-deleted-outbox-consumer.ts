...
export class TaskDeletedOutboxConsumer {
  ...
  private retryWakeAtMs: number | null = null;
  ...
  extends the backoff. Retry-backoff windows (cursor.retryBackoffUntil) report "waiting" and wake
  at their durable deadline; lease contention, fencing, and poll errors report "waiting" and reset
  the idle streak to the fast 5s base, so a transient failure recovers quickly instead of an error
  streak growing the delay to the 60s cap and hiding recovery behind it.
  */
  async start(): Promise<void> {
    if (this.running || !this.store.asyncLayer || !this.store.consumerId) return;
    this.running = true;
    this.idlePollsSinceEvent = 0;
   ...
   * next poll delay toward the 60s cap on "idle" only. Active and non-retry waiting outcomes reset
   * to the fast 5s base; a durable retry wait uses its exact deadline so transient failures recover
   * without repeated probes.
   */
  async poll(): Promise<TaskDeletedOutboxPollOutcome> {
    ...
    */
    ...
    try {
      ...
      // retried. This is a retry wait, not an idle outbox — the next probe wakes at its deadline.
      const retryBackoffUntilMs = cursor.retryBackoffUntil ? Date.parse(cursor.retryBackoffUntil) : Number.NaN;
      if (Number.isFinite(retryBackoffUntilMs) && retryBackoffUntilMs > Date.now()) {
        /*
        FNXC:TaskLifecycleConsumerIdleBackoff 2026-08-18-15:24:
        A retry window is a durable deadline, not an idle outbox. Wake at that exact deadline so
        a poison-event retry neither polls every 5s nor inherits the 60s idle cap.
        */
        this.retryWakeAtMs = retryBackoffUntilMs;
        return "waiting";
      }
   ...
   * reset the idle streak; a retry wait additionally supplies its durable wake-up deadline. An
   * error or retry streak must never masquerade as an idle streak and push the cadence toward 60s.
   */
  ...
  private nextPollDelayMs(): number {
    if (this.retryWakeAtMs !== null) {
      const remainingMs = this.retryWakeAtMs - Date.now();
